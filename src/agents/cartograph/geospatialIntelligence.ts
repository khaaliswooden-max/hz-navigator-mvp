...
export class CartographAgent {
  ...
  async verifyAddress(address: string): Promise<AddressVerificationResult> {
    ...
    const hubzoneStatus = await this.querySbaApi(
      geocoded.latitude as number,
      geocoded.longitude as number
    );
    ...
    const redesignationRisk = await this.calculateRedesignationRisk(
      ...
      censusTract.tractCode as string
    );
    ...
    return {
      address: {
        input: address,
        normalized: geocoded.normalizedAddress as string,
        street: geocoded.street as string,
        city: geocoded.city as string,
        state: geocoded.state as string,
        zipCode: geocoded.zipCode as string,
      },
      coordinates: {
        latitude: geocoded.latitude as number,
        longitude: geocoded.longitude as number,
      },
      hubzoneStatus: {
        isHubzone: hubzoneStatus.isHubzone,
        hubzoneType: hubzoneStatus.hubzoneType,
        censusTract: censusTract.tractCode as string,
        countyFips: censusTract.countyFips as string,
   ...
   */
  async geocodeAddress(address: string): Promise<Record<string, unknown>> {
